...
# 1. Target the ACTUAL Subway station table (usually Table 3 or 4)
url <- "https://en.wikipedia.org/wiki/List_of_MBTA_subway_stations"
page <- read_html(url)
all_tables <- page %>% html_table(fill = TRUE)

# We want the table that has "Station" and "City/neighborhood"
# Based on your output, we want to look for the table where the first column is 'Station'
stations_scraped <- all_tables[[3]] # Trying table 3

# 2. Fix your Facilities names
# Your names look like "Mini-high platform (Track 2/Boston)"
# We need to extract a location if possible, or use the 'id'
facilities_ready <- facilities_boston_only %>%
  mutate(
    # Let's try to extract any station names if they are hidden in the ID
    # Usually MBTA IDs look like 'place-north' or 'place-sstat'
...
# 2. Plot code to make it look pretty
ggplot() +
